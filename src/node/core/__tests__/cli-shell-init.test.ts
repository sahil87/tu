...
import { BASH_COMPLETION, ZSH_COMPLETION } from "../completions.js";
...
//
// The missing-arg contract runs the real CLI as a subprocess (pattern per
// cli-exit-codes.test.ts). The eval tests pipe the emitted script to
// `<shell> -c 'eval "$(cat)"'`, matching the standard's install idiom
// `eval "$(tu shell-init <shell>)"`. The zsh test is skip-guarded on PATH
...
describe("shell-init: emitted script is eval-safe in a subshell", () => {
  it("bash: eval of the emitted script exits 0", () => {
    const r = evalInSubshell("bash", BASH_COMPLETION);
  ...
  it("zsh: eval of the emitted script exits 0 (skipped when zsh is not on PATH)", (t) => {
    ...
    const r = evalInSubshell("zsh", ZSH_COMPLETION);
