...
- No product/runtime files change (no src/, no README.md, no docs/site/, no tests — there is nothing to test; this is a governance-document amendment). The only product file touched is `fab/project/constitution.md`; the fab change-tracking artifacts under `fab/changes/260717-43u4-bind-toolkit-standards/` (intake.md, plan.md, .status.yaml, .history.jsonl) are generated by the pipeline and are expected.
