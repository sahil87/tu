...
- `ccusage.ResolveBinary` (`src/go/internal/source/ccusage/exec.go`) **already** implements vendor-first resolution: `os.Executable()` → `filepath.EvalSymlinks` → `<dir>/vendor/ccusage/bin/ccusage`, then `ccusage` on PATH, then an `exec.ErrNotFound` error. It never probes `node_modules`. R1 changes it only by a pure refactor — extracting the vendor lookup into `resolveVendor(exe string) (string, bool)` so the symlink-chain layout can be tested (plan row R11), with identical resolution behavior; R1 produces the tarball layout it expects and proves the layout works through the Homebrew and `~/.local/bin` symlink chains.
