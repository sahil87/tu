...
| `null` | `0` |
| `true` / `false` | `1` / `0` |
| string | the StringNumericLiteral parse (`jsNumberString`): trimmed, empty → `0`; a `0x`/`0o`/`0b` prefix (no sign) → the integer in that base rounded to the nearest double via big.Float; else `strconv.ParseFloat`; failure → NaN |
| array | `Number()` of the `join(",")` string (`jsJoin`): `[5]` → `5`, `[]`/`[null]` → `0`, `[1,2]`/`[true]`/`[{}]` → NaN |
| object | NaN |
...
`sync.Runner` is `interface{ Run(dir string, args ...string) (stdout string, err error) }` — the one git verb the sync flow and the repair twin need; `Exec` satisfies it, tests use a recording fake or a PATH-first shell-script git. `Exec.Run` executes `git -C <dir> <args...>` with no timeout (the TS has none), stdout and stderr captured separately, each capped at `Exec.MaxBuffer` — the TS `maxBuffer`: zero selects `MaxBufferSync` (sync.ts's 10 MiB), `cmd/turepair` passes `MaxBufferRepair` (repair-metrics.mjs's 64 MiB), and a stream overflowing its cap kills the child, as Node does — and returns stdout on exit 0. On failure the error's `Error()` reproduces the TS `execFileAsync` wrapper byte for byte: `git -C <dir>... failed: {message}` where the message is `Command failed: git -C <dir> <args joined by single spaces>` + `"\n"` + captured stderr for a non-zero exit (the newline is unconditional — empty stderr still ends the message in `"\n"`, as Node emits), `{stdout|stderr} maxBuffer length exceeded` on a capture overflow (it replaces the Command failed text and fires even when the killed child exits 0), and `spawn git ENOENT` when git is not on PATH. Only the pull and post-retry push warnings surface this text; a commit failure carries it invisibly (DC-18).
