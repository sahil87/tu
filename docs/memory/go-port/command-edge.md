...
1. Normalizes: `req, notices, capActive := Normalize(req, deps.Now())`; then the scope check; then `context.WithTimeout(ctx, ccusage.DefaultTimeout)` once, for all tools.
