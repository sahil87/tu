...
### Requirement: MetricsDirGuard — the auto-clone fallback
`MetricsDirGuard(cfg Config, stateDir string, now time.Time, git Cloner) (Config, []string)` (guard.go — the TS `checkMetricsDirGuard`) returns the config the data path runs with (`Mode` possibly demoted to `Single`) plus the stderr lines the edge prints, in order; it prints nothing itself and is pure apart from the marker file and the clone it delegates to git. It fires only when `cfg.Mode == Multi` and `cfg.MetricsDir` does not exist — an existence check via `os.Stat`, not an is-repo check (the harness seed has no `.git/`). Then, in TS order: (1) `stateDir/.clone-failed` exists, its trimmed content parses as an RFC 3339 timestamp, and it is younger than 3 h (`cloneRetryWindow`, the TS `CLONE_RETRY_MS`) → return `cfg` with `Mode = Single` and the single line `Warning: metrics repo not available — falling back to single mode.`; a missing, unreadable, or unparseable marker counts as stale, a future timestamp as fresh. (2) Otherwise `git.CloneQuiet(ctx, cfg.MetricsRepo, cfg.MetricsDir)`: on success the config stays `Multi` (even when the directory still does not exist — the readers then find nothing), the marker is removed via `RemoveCloneMarker`, and the line is `Cloned metrics repo → {cfg.MetricsDir}` (absolute, DC-19); on failure `stateDir` is created if needed, the marker is written with content `now.UTC().Format("2006-01-02T15:04:05.000Z")` (the TS `toISOString()`), the config demotes to `Single`, and the line is `Warning: could not clone metrics repo ({detail}) — falling back to single mode.`, where `{detail}` reproduces Node's `execFileSync` message — `Command failed: git clone {url} {dir}` plus `\n` + the captured stderr when that is non-empty, or `spawnSync git ETIMEDOUT` when the error wraps `context.DeadlineExceeded`. The TS branch for an empty `metricsRepo` is unreachable (`Multi` implies a non-empty repo) and is not ported. `Cloner` is the one git question the guard asks — `interface{ CloneQuiet(ctx, url, dir) (stderr string, err error) }` — satisfied by `sync.Exec`.

#### Scenario: Marker cooldown, then clone
- **GIVEN** a Multi config whose `MetricsDir` is absent and a fake `Cloner`
- **WHEN** the guard runs with a marker written 1 h ago
- **THEN** the fake is never called, the returned config is `Single`, and the line is the `not available` warning
- **GIVEN** the same config with no marker (or a stale/garbage one) and a succeeding fake
- **WHEN** the guard runs
- **THEN** the fake is called once with `(cfg.MetricsRepo, cfg.MetricsDir)`, the config stays `Multi`, and the lines are exactly `[Cloned metrics repo → <dir>]`

### Requirement: sync.Exec drives the real git
`internal/sync/git.go` holds `type Exec struct{ MaxBuffer int }` (zero → `MaxBufferSync`, the sync.ts 10 MiB capture cap; `cmd/turepair` passes `MaxBufferRepair`, the mjs's 64 MiB), which satisfies `config.Git` and `config.Cloner`: `IsRepo(dir)` runs `git -C <dir> rev-parse --git-dir` with stdout/stderr discarded and reports exit 0; `Clone(ctx, url, dir, stdin, stdout, stderr)` runs `git clone <url> <dir>` with the reader and writers attached to the child's stdin, stdout and stderr (the TS `stdio: "inherit"`, so an interactive clone can still prompt for credentials), returning the exec error (`*exec.ExitError`) on a non-zero exit. `CloneQuiet(ctx, url, dir)` — the auto-clone guard's clone — runs the same argv with stdout/stderr captured (never inherited), `GIT_TERMINAL_PROMPT=0` appended to the child environment, and a 30 s deadline applied inside (the TS `timeout: 30_000`); `err` is nil on exit 0, the `*exec.ExitError` on a non-zero exit, and wraps `context.DeadlineExceeded` when the deadline fires. The interactive `Clone` deliberately carries neither bound — a credential prompt must survive it. The general call the sync flow and the repair twin make is `Run(dir string, args ...string) (stdout string, err error)` (the `Runner` interface): `git -C <dir> <args...>` with no timeout, stdout and stderr captured separately under the MaxBuffer cap, and Node-shaped error text on failure — `git -C <dir>... failed: Command failed: git -C <dir> <args>\n<stderr>` (the newline unconditional), `... failed: {stdout|stderr} maxBuffer length exceeded` on a capture overflow, or `... failed: spawn git ENOENT` ([metrics-sync](/go-port/metrics-sync.md)) (lsml). `git` resolves through `PATH` — how both sides reach the fake git in the harness, so the recorded argv shapes match the TS after home normalization.
...
`just go-diff --placeholder` reports 302 of 368 cases green (the full breakdown lives in [command-edge](/go-port/command-edge.md)). The config layer's slice: the 20 setup-command cases — `status`, `init-conf`, `init-metrics`, `init-metrics-url`, `init-metrics-extra` × the four conf variants — the two `--dry-run` misuse cases, and the `envrepo` axis (32/32 — `TU_METRICS_REPO` set with the dir absent), which exercises `MetricsDirGuard`'s quiet clone: one `git clone <url> <dir>` call, the `Cloned metrics repo → <abs dir>` stderr line, then a live-only render. The `[calls differ]` marker is absent from the `init-metrics` groups: both sides issue the same git argv after home normalization. See [differential-harness](/harness/differential-harness.md).
...
**Decision**: `internal/sync/git.go` holds the real git driver (`Exec`) that satisfies `config.Git` and `config.Cloner`; the metrics-repo writer, never-shrink guard, dry-run report, and sync flow grow the same package in B6.
...
**Why**: The only-writer rule; the `Git`/`Exec` precedent from `InitMetrics`; a fake `Cloner` makes every branch (marker fresh/stale/garbage, clone ok/fail/timeout) unit-testable without git or a network.
...
**Decision**: `command.Deps.LastSync func() string` is built at the edge over `config.LastSync(StateDir(home), time.Now())` and called only by the `lb` path; a nil `LastSync` in tests reads as `"never"`.
**Why**: `command` stays I/O-free (the target architecture); the closure mirrors `Deps.Now`; no other command pays the file read; `Status` and the leaderboard footer share one formatting implementation.
