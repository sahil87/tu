---
type: memory
description: "Go release pipeline — four cross-compiled tu-go-<os>-<arch>.tar.gz assets (flat: tu + vendored platform ccusage + tu.default.conf) plus tu-go-SHA256SUMS, the CCUSAGE_VERSION sole pin, npm-registry curl fetch, the generated dist/tu.rb formula (libexec + bin symlink, no depends_on) that release.yml copies over the tap's Formula/tu.rb as its last step, the rollback path (a higher-numbered release whose formula points at a pre-Z1 tag; brew never downgrades), and the per-PR cross-compile lane"
---
# Go Release Pipeline
...
### Requirement: `CCUSAGE_VERSION` is the sole ccusage pin
The repo-root file `CCUSAGE_VERSION` holds the bare ccusage version on one newline-terminated line (`20.0.19`). `scripts/package-go.sh` reads it **first, before any network fetch**. Bumping ccusage means editing `CCUSAGE_VERSION` and nothing else (0118, 6wpm).

### Requirement: ccusage fetch from the npm registry
For each platform the script maps Go arch to npm arch — `amd64`→`x64`, `arm64` unchanged, os unchanged — and fetches `https://registry.npmjs.org/@ccusage/ccusage-<npm_os>-<npm_arch>/-/ccusage-<npm_os>-<npm_arch>-<CCUSAGE_VERSION>.tgz` with `curl -fsSL` into `dist/ccusage-tarballs/`, skipping a tarball that is already cached there. A non-2xx response removes the partial file and fails the script with `error: failed to fetch <url>` on stderr. Exactly `package/bin/ccusage` is extracted from the tarball into the staging tree. No node/npm is involved in the fetch — `curl` only (0118).
...
The `release` job runs in exactly two cases: a `v*` tag push and `workflow_dispatch` (which runs `scripts/release.sh "${{ inputs.bump }}"` — tag only, no commit — then reads the new tag back via `git tag --sort=-v:refname | head -1`); its checkout uses `fetch-depth: 0` and `ref:` the pushed tag or `main` on dispatch. It runs `actions/setup-go` (same pinned SHA as `ci.yml`, `go-version-file: src/go/go.mod`, `cache: false`), then `just go-build-all`, `just go-package`, and `just go-formula "<tag>"` + `cat dist/tu.rb` + `just release-notes "<tag>"` — all **before** "Create GitHub Release". No step carries `continue-on-error`: a Go failure aborts the run before the Release or the tap is touched (the job's fail-loud-first ordering). `gh release create` passes the five assets (`dist/tu-go-darwin-arm64.tar.gz`, `dist/tu-go-darwin-amd64.tar.gz`, `dist/tu-go-linux-arm64.tar.gz`, `dist/tu-go-linux-amd64.tar.gz`, `dist/tu-go-SHA256SUMS`). The job's **last** step, "Update Homebrew tap", clones `sahil87/homebrew-tap` with `HOMEBREW_TAP_TOKEN`, runs `cp dist/tu.rb /tmp/tap/Formula/tu.rb`, commits as `github-actions[bot]` with message `tu <version>`, and pushes — fab-kit's tap step verbatim. It runs after the Release on purpose: the formula's tarball URLs resolve only once the assets exist. The step is shared by both entry paths and carries no bump-type or version-shape guard — every release pushes the Go formula. `tu-go-*` assets exist only for releases from v0.11.6 on; the Go formula reaches the tap from the cutover release, 0.13.0 (0118, jmh4, 6wpm).
...
### Requirement: Rollback is a higher-numbered release whose formula points at a pre-Z1 tag
`tu update` runs `brew upgrade tu`, and Homebrew never downgrades an installed keg: reverting the tap's `Formula/tu.rb` to the Node formula at `tag: "v0.12.0"` leaves every machine that already installed 0.13.0 on the Go binary (the upgrade is a no-op that reports success). Rolling users back through the normal update path therefore requires a **release whose version is higher than the Go one and whose tap formula is the Node formula**: cut `just release` (e.g. 0.13.1) and point its formula at a pre-Z1 tag — the pre-Z1 tags carry the retired TypeScript tree in their own git history, so the old formula there still builds, and nothing in `main` is needed. A tap revert on its own only protects machines that have not yet upgraded; a machine already on 0.13.0 needs an explicit `brew reinstall tu` after the revert (jmh4, 6wpm).
...
- **THEN** `tu update` on a machine already at 0.13.0 leaves the Go binary installed (`brew upgrade` does not downgrade); only `brew reinstall tu` — or a 0.13.1 release whose formula points at a pre-Z1 tag — moves it back
...
**Why**: The plan names `dist/` for the formula; fab-kit uses `dist/bin/`; tu is not published to a package registry so no `files`-sweep concern applies to release artifacts.
...
**Why**: The fetch needs no runtime beyond curl; `tu-go-SHA256SUMS` over the repo's own archives is what installs verify; the registry's integrity field comes from the same source.
...
**Why**: The tag-push path carries no bump type; an `X.Y.0` check would need tap state to know it is the first Go push; and any in-job guard fires after `scripts/release.sh` has already pushed the `v*` tag, leaving a dangling tag — a worse mess than a Go formula under a patch number, which is a versioning-policy nit rather than a broken install.
