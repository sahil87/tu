...
- The active drop count MUST scale with the rain-zone height, not width alone: `computeActiveDropCount(cols, rows)` (exported from `rain.ts`) returns `round(cols × DENSITY × heightScale)` where `heightScale = min(MAX_DENSITY_SCALE, max(1, rows / DENSITY_REF_ROWS))`, with `DENSITY = 0.3`, `DENSITY_REF_ROWS = 20`, `MAX_DENSITY_SCALE = 3`. `initDrops()` calls the helper and assigns drops round-robin over the shuffled column list (`columns[i % cols]`)
- At or below `DENSITY_REF_ROWS` (20 rows) the count MUST equal the prior width-only `round(cols × DENSITY)` — the `max(1, …)` clamp keeps the common laptop-terminal case byte-identical (one drop per column). Taller zones scale linearly up to the `MAX_DENSITY_SCALE` (3×) cap at 60+ rows, restoring roughly constant cell coverage (~8%) instead of coverage decaying as ~1.65/rows
...
- **Height-scaled drop count** (`rain-density-scale-height`): the active drop count scales with rain-zone height rather than width alone, because `DENSITY` was calibrated at `DENSITY_REF_ROWS` (20) and fixed 3–8-cell trails meant coverage decayed as ~1.65/rows — a 60-row zone showed ~2.75% coverage versus ~11% at 15 rows. Scaling the *count* (not trail length or speed) preserves the calibrated per-drop look while restoring roughly constant coverage; the renderer needed no changes because it already tracks cell positions in a `"row,col"`-keyed Set with last-wins overlaps and clears only vacated cells. The scale is clamped to 1× at/below the reference height (byte-identical output for the common small-zone case) and capped at `MAX_DENSITY_SCALE` (3×) to bound per-frame ANSI output on very tall terminals — stdout backpressure handling remains deferred, so the cap keeps this change from reopening that risk. Column assignment is round-robin over the shuffled column list, so multi-drop columns engage only if the constants are retuned such that `DENSITY × MAX_DENSITY_SCALE > 1` (with the shipped 0.3 × 3 = 0.9 the count never exceeds `cols`, so every column hosts at most one drop). The guard-removal is what enables multi-drop columns; the shipped constants exercise the round-robin only up to one full pass.
